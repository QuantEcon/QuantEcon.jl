...
mutable struct RegimeSwitchingModel{TD <: AbstractArray, 
    ...
    function RegimeSwitchingModel(g, y, parameter, P, M, prob_smoothed, logL)
        ...
        return new{typeof(y), typeof(P), typeof(prob_smoothed), typeof(parameter)}(g, y, parameter,
                                                      P, M, prob_smoothed, logL)
    end
end
...
##### Arguments
- `rsm::RegimeSwitchingModel`: `RegimeSwitchingModel` specifying the model.
- `p_s_joint_smoothed::AbstractArray`: `T x n_state x n_state` array that stores joint distribution 
                               of state at period `t` and `t+1` data conditional on 
                               the all information.
